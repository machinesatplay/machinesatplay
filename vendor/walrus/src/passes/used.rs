...
impl Used {
    ...
    pub fn new(module: &Module) -> Used {
        ...
        #[cfg(feature = "parallel")]
        // large modules spend most gc time walking function bodies serially.
        // index body references in parallel, then traverse the graph as usual.
        // small modules keep the allocation-free path.
        let function_references: Option<IdHashMap<Function, FunctionReferences>> =
            (module.funcs.iter_local().count() >= 4096).then(|| {
                module
                    .funcs
                    .par_iter_local()
                    .map(|(id, function)| {
                        ...
                        visitor.local_ids.sort_unstable_by_key(|id| id.index());
                        visitor.local_ids.dedup();
                        for id in std::mem::take(&mut visitor.local_ids) {
                            if let crate::ValType::Ref(ref_type) = module.locals.get(id).ty() {
                                visitor.push_ref_type(&ref_type);
                            }
                        }
                        visitor.references.sort_unstable_by_key(Reference::key);
                        visitor.references.dedup_by_key(|reference| reference.key());
                        (
                            id,
                            FunctionReferences {
            ...
            while let Some(f) = stack.funcs.pop() {
                ...
                match &func.kind {
                    FunctionKind::Local(func) => {
                        #[cfg(feature = "parallel")]
                        if let Some(function_references) = &function_references {
                            for reference in &function_references[&f].references {
                                match reference {
                                    Reference::Function(id) => {
                                        stack.push_func(*id);
                                    }
                                    Reference::Memory(id) => {
                                        stack.push_memory(*id);
                                    }
                                    Reference::Global(id) => {
                                        stack.push_global(*id);
                                    }
                                    Reference::Table(id) => {
                                        stack.push_table(*id);
                                    }
                                    Reference::Type(id) => {
                                        stack.used.types.insert(*id);
                                    }
                                    Reference::Data(id) => {
                                        stack.push_data(*id);
                                    }
                                    Reference::Element(id) => {
                                        stack.push_element(*id);
                                    }
                                    Reference::Tag(id) => {
                                        stack.push_tag(*id);
                                    }
...
impl Reference {
    fn key(&self) -> (u8, usize) {
        match self {
            Reference::Function(id) => (0, id.index()),
            Reference::Memory(id) => (1, id.index()),
            Reference::Global(id) => (2, id.index()),
            Reference::Table(id) => (3, id.index()),
            Reference::Type(id) => (4, id.index()),
            Reference::Data(id) => (5, id.index()),
            Reference::Element(id) => (6, id.index()),
            Reference::Tag(id) => (7, id.index()),
        }
    }
}
...
#[cfg(feature = "parallel")]
impl<'expr> Visitor<'expr> for ReferenceVisitor {
    fn visit_function_id(&mut self, &id: &FunctionId) {
        self.references.push(Reference::Function(id));
    }

    fn visit_memory_id(&mut self, &id: &MemoryId) {
        self.references.push(Reference::Memory(id));
    }

    fn visit_global_id(&mut self, &id: &GlobalId) {
        self.references.push(Reference::Global(id));
    }

    fn visit_table_id(&mut self, &id: &TableId) {
        self.references.push(Reference::Table(id));
    }

    fn visit_type_id(&mut self, &id: &TypeId) {
        self.references.push(Reference::Type(id));
    }
    ...
    fn visit_data_id(&mut self, &id: &DataId) {
        self.references.push(Reference::Data(id));
    }

    fn visit_element_id(&mut self, &id: &ElementId) {
        self.references.push(Reference::Element(id));
    }

    fn visit_tag_id(&mut self, &id: &TagId) {
        self.references.push(Reference::Tag(id));
    }

    fn visit_select(&mut self, instr: &Select) {
        if let Some(crate::ValType::Ref(ref_type)) = &instr.ty {
            self.push_ref_type(ref_type);
        }
    }

    fn visit_ref_null(&mut self, instr: &RefNull) {
        self.push_ref_type(&instr.ty);
    }
}

#[cfg(feature = "parallel")]
impl ReferenceVisitor {
    fn push_ref_type(&mut self, ref_type: &RefType) {
        if let HeapType::Concrete(id) | HeapType::Exact(id) = ref_type.heap_type {
            self.references.push(Reference::Type(id));
        }
    }
}
