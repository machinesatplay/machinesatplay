...
mod tests {
    ...
    #[test]
    fn parallel_reference_index_preserves_a_large_call_chain() {
        ...
        for _ in 0..4097 {
            ...
            if let Some(previous) = previous {
                builder.func_body().call(previous);
            }
            previous = Some(builder.finish(Vec::new(), &mut module.funcs));
        }

        let dead = FunctionBuilder::new(&mut module.types, &[], &[]);
        dead.finish(Vec::new(), &mut module.funcs);
        module.exports.add("root", previous.unwrap());

        run(&mut module);

        assert_eq!(module.funcs.iter().count(), 4097);
    }
}
