...
impl Module {
    ...
    pub(crate) fn parse_local_functions(
        ...
        // Legacy catch block types must exist before function bodies are
        // decoded in parallel. Discover them in parallel too: large modules
        // can contain hundreds of thousands of functions, and scanning every
        // body here was otherwise a substantial serial prefix.
        let legacy_catch_block_types = maybe_parallel!(functions.(iter | par_iter))
            .map(|(body, _)| self.legacy_catch_block_types(body, indices))
            .collect::<Result<Vec<_>>>()?;
        for (params, results) in legacy_catch_block_types.into_iter().flatten() {
    ...
    /// Find the implicit `[tag params] -> [try results]` `Type` of every
    /// non-`Simple` legacy `catch` / `catch_all` handler in `body`.
    ///
    ...
    fn legacy_catch_block_types(
        &self,
        body: &wasmparser::FunctionBody,
        indices: &IndicesToIds,
    ) -> Result<Vec<(Vec<ValType>, Vec<ValType>)>> {
...
fn used_local_functions<'a>(cx: &mut EmitContext<'a>) -> Vec<(FunctionId, &'a LocalFunction, u64)> {
    ...
    let module_functions = &cx.module.funcs;
    let mut functions = maybe_parallel!(module_functions.(iter | par_iter))
        .filter_map(|f| match &f.kind {
            FunctionKind::Local(l) => Some((f.id(), l, l.size())),
    ...
    // longer to compile.
    functions.sort_unstable_by_key(|(id, _, size)| (cmp::Reverse(*size), *id));
...
impl Emit for ModuleFunctions {
    fn emit(&self, cx: &mut EmitContext) {
        ...
        let retain_local_names = !cx.module.config.skip_name_section;
        ...
        let bytes = maybe_parallel!(functions.(into_iter | into_par_iter))
            .map(|(id, func, _size)| {
                ...
                let locals = retain_local_names.then_some((used_locals, local_indices));
                (wasm, wasm_function.byte_len(), id, locals, map)
            })
            .collect::<Vec<_>>();

        if retain_local_names {
            cx.indices.locals.reserve(bytes.len());
            cx.locals.reserve(bytes.len());
        }

        let mut offset_data = generate_map.then(|| Vec::with_capacity(bytes.len()));
        for (wasm, byte_len, id, locals, map) in bytes {
            let leb_len = wasm.len() - byte_len;
            wasm_code_section.raw(&wasm[leb_len..]);
            if let Some((used_locals, local_indices)) = locals {
                cx.indices.locals.insert(id, local_indices);
                cx.locals.insert(id, used_locals);
            }
            if let Some(offset_data) = &mut offset_data {
                offset_data.push((byte_len, id, map, leb_len));
            }
        }
        cx.wasm_module.section(&wasm_code_section);

        let Some(offset_data) = offset_data else {
            return;
        };
...
mod tests {
    use super::*;
    use crate::{ElementItems, ElementKind, Export, FunctionBuilder, Module};
    ...
    #[test]
    fn parses_multivalue_legacy_catch() {
        use wasm_encoder::{
            BlockType, CodeSection, Function, FunctionSection, Instruction, Module as WasmModule,
            TagKind, TagSection, TagType, TypeSection, ValType as WasmValType,
        };
        ...
        types
            .ty()
            .function([WasmValType::I32, WasmValType::I64], []);
        types
            .ty()
            .function([], [WasmValType::I32, WasmValType::I64]);
        wasm.section(&types);
        ...
        function
            .instruction(&Instruction::Try(BlockType::FunctionType(1)))
            .instruction(&Instruction::I32Const(1))
            .instruction(&Instruction::I64Const(2))
            .instruction(&Instruction::Catch(0))
        ...
        let mut module = crate::ModuleConfig::new().parse(&wasm.finish()).unwrap();
        let emitted = module.emit_wasm();
        let mut features = wasmparser::WasmFeatures::default();
        features.insert(wasmparser::WasmFeatures::LEGACY_EXCEPTIONS);
        wasmparser::Validator::new_with_features(features)
            .validate_all(&emitted)
            .unwrap();
    }

    #[test]
    fn emission_declares_functions_referenced_by_code() {
        let mut module = Module::default();

        let target =
            FunctionBuilder::new(&mut module.types, &[], &[]).finish(Vec::new(), &mut module.funcs);
        let mut caller = FunctionBuilder::new(&mut module.types, &[], &[]);
        caller
            .func_body()
            .instr(crate::ir::RefFunc { func: target })
            .drop();
        caller.finish(Vec::new(), &mut module.funcs);

        let emitted = module.emit_wasm();
        assert!(module.elements.iter().any(|element| {
            matches!(&element.kind, ElementKind::Declared)
                && matches!(
                    &element.items,
                    ElementItems::Functions(functions) if functions.contains(&target)
                )
        }));
