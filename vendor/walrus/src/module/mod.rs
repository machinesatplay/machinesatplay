...
pub struct Module {
    ...
    pub(crate) original_indices: Option<IndicesToIds>,
    pub(crate) emitted_indices: Option<crate::OriginalToEmittedIndices>,
}
...
impl Module {
    ...
    fn parse(wasm: &[u8], config: &ModuleConfig) -> Result<Module> {
        ...
        ret.original_indices = Some(indices);
    ...
    fn ensure_func_declarations(&mut self) {
        ...
        fn collect_local_ref_funcs(set: &mut IdHashSet<crate::Function>, local: &LocalFunction) {
            let mut visitor = RefFuncVisitor { set };
            dfs_in_order(&mut visitor, local, local.entry_block());
        }

        // 1. Collect all functions referenced by ref.func in code.
        #[cfg(feature = "parallel")]
        let mut ref_funcs = self
            .funcs
            .par_iter_local()
            .fold(IdHashSet::default, |mut set, (_, local)| {
                collect_local_ref_funcs(&mut set, local);
                set
            })
            .reduce(IdHashSet::default, |mut set, other| {
                set.extend(other);
                set
            });
        ...
        let mut ref_funcs = {
            ...
            for (_, local) in self.funcs.iter_local() {
                collect_local_ref_funcs(&mut set, local);
            }
            set
        };
    ...
    pub fn emit_wasm(&mut self) -> Vec<u8> {
        ...
        let emitted_function_imports = self
            .funcs
            .iter()
            .filter(|function| matches!(function.kind, FunctionKind::Import(_)))
            .count() as u32;
        ...
        let emitted_indices = self
            .original_indices
            .as_ref()
            .map(|original| original.to_emitted(&indices, emitted_function_imports));
        let out = cx.wasm_module.finish();
        self.emitted_indices = emitted_indices;
    ...
    /// Returns the mapping from indices in the parsed module to indices in
    /// the latest emitted module.
    pub fn original_to_emitted_indices(&self) -> Option<&crate::OriginalToEmittedIndices> {
        self.emitted_indices.as_ref()
    }
