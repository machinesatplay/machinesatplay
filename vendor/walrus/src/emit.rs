...
impl IdsToIndices {
    pub(crate) fn table_index(&self, id: TableId) -> Option<u32> {
        self.tables.get(&id).copied()
    }

    pub(crate) fn type_index(&self, id: TypeId) -> Option<u32> {
        self.types.get(&id).copied()
    }

    pub(crate) fn function_index(&self, id: FunctionId) -> Option<u32> {
        self.funcs.get(&id).copied()
    }

    pub(crate) fn global_index(&self, id: GlobalId) -> Option<u32> {
        self.globals.get(&id).copied()
    }

    pub(crate) fn memory_index(&self, id: MemoryId) -> Option<u32> {
        self.memories.get(&id).copied()
    }

    pub(crate) fn element_index(&self, id: ElementId) -> Option<u32> {
        self.elements.get(&id).copied()
    }

    pub(crate) fn data_index(&self, id: DataId) -> Option<u32> {
        self.data.get(&id).copied()
    }

    pub(crate) fn tag_index(&self, id: TagId) -> Option<u32> {
