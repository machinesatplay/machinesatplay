...
pub fn detect_exception_handling_version(
    ...
    fn inspect(local: &walrus::LocalFunction) -> u8 {
        let mut detector = EhDetector {
            has_try: false,
            has_try_table: false,
        };
        walrus::ir::dfs_in_order(&mut detector, local, local.entry_block());
        u8::from(detector.has_try) * TRY + u8::from(detector.has_try_table) * TRY_TABLE
    }

    let local_count = module.funcs.iter_local().count();
    let flags = if local_count >= 256 {
        module
            .funcs
            .par_iter_local()
            .map(|(_, local)| inspect(local))
            .reduce(|| 0, |left, right| left | right)
    } else {
        let mut flags = 0;
        for (_, local) in module.funcs.iter_local() {
            flags |= inspect(local);
            if flags == (TRY | TRY_TABLE) {
                break;
            }
        }
        flags
...
mod tests {
    ...
    #[test]
    fn detect_eh_version_in_large_module() {
        let mut wat = String::from("(module");
        for _ in 0..300 {
            wat.push_str("(func i32.const 1 drop)");
        }
        wat.push_str("(func (block $catch (try_table (catch_all $catch))))");
        wat.push(')');
        let module = parse_wat(&wat);
        assert_eq!(
            detect_exception_handling_version(&module, false),
