...
#[cfg(not(test))]
const PARALLEL_REWRITE_THRESHOLD: usize = 4096;
#[cfg(test)]
const PARALLEL_REWRITE_THRESHOLD: usize = 0;
...
impl Transform<'_> {
    ...
    fn rewrite_calls(&mut self, module: &mut Module) -> Result<(), Error> {
        let clone_ref = self.clone_ref()?;
        let heap_dealloc = self.heap_dealloc()?;
        let function_ids = module
            .funcs
            .iter_local()
            .map(|(id, _)| id)
            .filter(|id| !self.shims.contains(id))
            .collect::<Vec<_>>();
        let scratch_by_function = function_ids
            .iter()
            .map(|id| (*id, module.locals.add(ValType::I32)))
            .collect::<HashMap<_, _>>();

        let rewrite = |id: FunctionId, func: &mut walrus::LocalFunction| {
            ...
            dfs_pre_order_mut(
                &mut Rewrite {
                    ...
                    heap_dealloc,
                    xform: self,
                    scratch_i32: scratch_by_function[&id],
                },
                func,
                entry,
            );
        ...
        if function_ids.len() >= PARALLEL_REWRITE_THRESHOLD {
            module
                .funcs
                .par_iter_local_mut()
                .filter(|(id, _)| !self.shims.contains(id))
                .for_each(|(id, func)| rewrite(id, func));
        } else {
            module
                .funcs
                .iter_local_mut()
