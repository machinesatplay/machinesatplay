...
fn gc_module_and_adapters(module: &mut Module) {
    loop {
        ...
        // things.
        let adapters_with_core_roots = section
            .adapters
            .iter()
            .filter_map(|(id, adapter)| adapter_has_core_gc_roots(adapter).then_some(*id))
            .collect::<Vec<_>>();
        let any_removed = section.gc();
        let removed_core_roots = adapters_with_core_roots
            .iter()
            .any(|id| !section.adapters.contains_key(id));
        module.customs.add(*section);
        if !any_removed || !removed_core_roots {
            break;
        }
    }
}

/// Returns whether an adapter contributes roots to walrus's core Wasm graph.
///
/// Adapter GC can remove adapter-only calls without changing any core roots. In
/// that case, another full walrus GC pass cannot delete anything and is wasted
/// work. Keep this list in sync with `NonstandardWitSection::add_gc_roots`.
fn adapter_has_core_gc_roots(adapter: &wit::Adapter) -> bool {
    use wit::{AdapterKind, Instruction};
    ...
    instructions.iter().any(|instruction| {
        matches!(
            ...
            Instruction::DeferFree { .. }
                | Instruction::StoreRetptr { .. }
                | Instruction::LoadRetptr { .. }
                | Instruction::View { .. }
                | Instruction::OptionView { .. }
                | Instruction::MemoryToString(_)
                | Instruction::VectorToMemory { .. }
                | Instruction::OptionVector { .. }
                ...
                | Instruction::I32FromOptionExternref {
                    table_and_alloc: Some(_)
                }
                | Instruction::UnwrapResult {
                    table_and_drop: Some(_)
                }
...
#[cfg(test)]
mod gc_adapter_tests {
    use super::*;
    use wit::{Adapter, AdapterId, AdapterKind, Instruction, InstructionData, StackChange};

    fn local_adapter(instruction: Instruction) -> Adapter {
        Adapter {
            id: AdapterId(0),
            params: Vec::new(),
            results: Vec::new(),
    ...
    #[test]
    fn adapter_only_calls_do_not_require_core_gc() {
        let adapter = local_adapter(Instruction::CallAdapter(AdapterId(1)));
        assert!(!adapter_has_core_gc_roots(&adapter));
    }

    #[test]
    fn removed_core_roots_still_reach_a_gc_fixpoint() {
        let mut module = Module::default();
        let builder = walrus::FunctionBuilder::new(&mut module.types, &[], &[]);
        let function = builder.finish(Vec::new(), &mut module.funcs);

        let mut section = wit::NonstandardWitSection::default();
        section.adapters.insert(
            AdapterId(0),
            local_adapter(Instruction::DeferFree {
                free: function,
                align: 1,
            }),
        );
