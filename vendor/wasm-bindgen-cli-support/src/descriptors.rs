...
#[cfg(not(test))]
const PARALLEL_CAST_SCAN_THRESHOLD: usize = 4096;
#[cfg(test)]
const PARALLEL_CAST_SCAN_THRESHOLD: usize = 0;
...
impl WasmBindgenDescriptorsSection {
    ...
    fn execute_casts(
        ...
        let local_functions = module.funcs.iter_local().collect::<Vec<_>>();
        let find_cast = |(func_id, local): &(FunctionId, &walrus::LocalFunction)| {
            let mut find = FindDescribeCast {
                wbindgen_describe_cast,
                found: false,
            };
            dfs_in_order(&mut find, local, local.entry_block());
            find.found.then_some(*func_id)
        };
        let replace_with_imports = if local_functions.len() >= PARALLEL_CAST_SCAN_THRESHOLD {
            local_functions
                .par_iter()
                .filter_map(find_cast)
                .collect::<Vec<_>>()
        } else {
            local_functions.iter().filter_map(find_cast).collect()
        };
